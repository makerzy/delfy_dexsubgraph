...
import { updatePairDayData, updateTokenDayData, updateDelfyDayData, updatePairHourData } from './dayUpdates'
...
export function handleTransfer(event: Transfer): void {
  ...
  let factory = DelfyFactory.load(FACTORY_ADDRESS)
...
export function handleSync(event: Sync): void {
  ...
  let delfy = DelfyFactory.load(FACTORY_ADDRESS)

  // reset factory liquidity by subtracting onluy tarcked liquidity
  delfy.totalLiquidityBNB = delfy.totalLiquidityBNB.minus(pair.trackedReserveBNB as BigDecimal)
  ...
  pair.trackedReserveBNB = trackedLiquidityETH
  ...
  delfy.totalLiquidityBNB = delfy.totalLiquidityBNB.plus(trackedLiquidityETH)
  delfy.totalLiquidityUSD = delfy.totalLiquidityBNB.times(bundle.bnbPrice)
...
export function handleMint(event: Mint): void {
  ...
  delfy.txCount = delfy.txCount.plus(ONE_BI)
...
export function handleSwap(event: Swap): void {
  ...
  let derivedAmountETH = token1.derivedBNB
    .times(amount1Total)
  ...
  let delfy = DelfyFactory.load(FACTORY_ADDRESS)
  delfy.totalVolumeUSD = delfy.totalVolumeUSD.plus(trackedAmountUSD)
  delfy.totalVolumeBNB = delfy.totalVolumeBNB.plus(trackedAmountETH)
  delfy.untrackedVolumeUSD = delfy.untrackedVolumeUSD.plus(derivedAmountUSD)
  delfy.txCount = delfy.txCount.plus(ONE_BI)
  ...
  let delfyDayData = updateDelfyDayData(event)
  let token0DayData = updateTokenDayData(token0 as Token, event)
  let token1DayData = updateTokenDayData(token1 as Token, event)

  // swap specific updating
  delfyDayData.dailyVolumeUSD = delfyDayData.dailyVolumeUSD.plus(trackedAmountUSD)
  delfyDayData.dailyVolumeBNB = delfyDayData.dailyVolumeBNB.plus(trackedAmountETH)
  delfyDayData.dailyVolumeUntracked = delfyDayData.dailyVolumeUntracked.plus(derivedAmountUSD)
  delfyDayData.save()
